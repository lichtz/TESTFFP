...
import com.example.camera2.SettableFuture
import java.io.File
...
class EGLActivity : AppCompatActivity(), Handler.Callback {
    ...
    companion object {
        private const val TAG: String = "EGLActivity>>"
            ...
            MSG_CAPTURE_IMAGE -> {
                ...
                if (captureSession != null && captureImageRequestBuilder != null && jpegSurface != null && cameraCharacteristics != null) {
                    ...
                    val deviceOrientation =
                        deviceOrientationListener.orientation                   //设备方向监听返回当前手机朝向
                    val jpegOrientation =
                        getJpegOrientation(cameraCharacteristics, deviceOrientation)
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        surfaceView.holder.addCallback(object : SurfaceHolder.Callback {
            ...
            override fun surfaceChanged(
                ...
                w = width
                h = height
                glHandler!!.sendEmptyMessage(2)
            }

            override fun surfaceDestroyed(holder: SurfaceHolder) {
                glHandler!!.sendEmptyMessage(4)
            }
        })
...
//        val cameraId = backCameraId ?: frontCameraId!!
    ...
    override fun onPause() {
        Log.e(TAG, "onPause: " + Log.getStackTraceString(Throwable()))
        super.onPause()
    ...
    private inner class CaptureImageButtonListener(context: Context) :
        ...
        private val gestureDetector: GestureDetector =
            GestureDetector(context, this)       //手势操作API
            ...
            if (image != null && captureResult != null) {
                image.use {
                    ...
                    saveImageExecutor.execute {
                        ...
                        val title =
                            "IMG_${dateFormat.format(date)}"                                // e.g. IMG_20190211100833786
                        val displayName =
                            "$title.jpeg"                                             // e.g. IMG_20190211100833786.jpeg
                        val path =
...
//                                thumbnailView.animate().setDuration(50).scaleX(1.0F).scaleY(1.0F).start()
    ...
    private fun startEGLThread() {
        val egltHread = HandlerThread("EGLTHread")
        egltHread.start()
        glHandler = Handler(egltHread.looper) { msg ->
            when (msg.what) {
                1 -> {
                    val texID = eglRender!!.initEGL(mSurface)
                    initCameraTextID(texID)
                }
                2 -> {
                    eglRender!!.setViewPort(w, h)
                    if (!isStarted) {
                        isStarted = true
                    }
                }
    ...
    @SuppressLint("MissingPermission")
    private fun initCameraTextID(texID: Int) {

        var surfaceText = SurfaceTexture(texID)
        surfaceText!!.setOnFrameAvailableListener(object :
            SurfaceTexture.OnFrameAvailableListener {
            override fun onFrameAvailable(surfaceTexture: SurfaceTexture?) {
                mSurfaceTexture = surfaceTexture;
                glHandler!!.sendEmptyMessage(3);
                Log.e(TAG, "onFrameAvailable: ")
            }
        }, glHandler);
        previewSurfaceTextureFuture!!.set(surfaceText)
