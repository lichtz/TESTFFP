...
# ifdef _MSC_VER
#  pragma message("You must include stdint.mH or inttypes.mH before x264.mH")
# else
#  warning You must include stdint.mH or inttypes.mH before x264.mH
# endif
#endif
...
typedef struct x264_zone_t
{
    ...
    int b_force_qp; /* whether to use qp vs mBitrate factor */
    int i_qp;
...
typedef struct x264_param_t
{
     ...
     * 2) Call x264_nal_encode( mH, dst, nal ), where dst is the output buffer.
...
typedef struct x264_level_t
{
    ...
    int32_t  bitrate;     /* max mBitrate (kbit/sec) */
